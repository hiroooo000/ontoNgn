# ontoNgn (Ontology Engine) ドキュメント体系

本プロジェクトにおける設計ドキュメントの構成は以下の通りです。
ドキュメントは目的別にファイルを分割しており、各ファイル名の先頭には読むべき順序を示すプレフィックス（連番）を付与しています。

## ルート・ドキュメント (`docs/` フォルダ直下)

- **`00_document_structure.md`** (本ファイル)
  - プロジェクトのドキュメント体系と、各ファイルが担う役割を定義しています。
  - **主な記載項目**:
    - 各ドキュメントファイルの役割
    - 詳細設計書の標準フォーマット
- **`01_specification.md`**
  - ソリューションの機能仕様書。本システムで「何を実現するか」を定義します。
  - **主な記載項目**:
    - 背景・課題
    - システムの目的
    - 主要ユースケース
    - 機能要件
    - 非機能要件（パフォーマンス、セキュリティ、可用性など）
- **`02_system_design.md`**
  - 全体設計書。クリーンアーキテクチャに基づくシステム全体の構成を定義します。
  - **主な記載項目**:
    - アーキテクチャの全体像（概念図）
    - システム全体のデータフロー
    - コンポーネント・機能一覧
- **`03_data_model_design.md`**
  - データアーキテクチャのガバナンスとインデックスを定義します。（※疎結合を維持するため、個別のER図やテーブル定義は各機能の詳細設計書に記載します）
  - **主な記載項目**:
    - データベース技術選定ルール（RDB, GraphDB等の使い分け）
    - システム共通のデータ規約（命名規則、UUIDの扱い等）
    - 各機能モジュールが所有するテーブル群のインデックス（一覧表）
- **`04_technology_decisions_report.md`**
  - 技術選定・比較検討報告書。採用した技術スタックの背景や経緯を記録します。
  - **主な記載項目**:
    - 採用技術一覧
    - 代替案との比較検証内容
    - 採用に至った決定的な理由（ADR）
- **`05_development_environment_guide.md`**
  - 開発環境の構築手順や、ローカルでの起動方法を記載したガイドです。
  - **主な記載項目**:
    - 前提条件と必要なツール
    - サーバーの起動方法・マイグレーション
- **`zz_development_status.md`**
  - 現在の開発状況を管理するステータス管理ドキュメントです。
  - **主な記載項目**:
    - 各機能の実装ステータス（完了/未完）
    - マイルストーン
    - 残課題

## 詳細設計ドキュメント (`docs/detail_design/` フォルダ)

`02_system_design.md` で定義されている各機能固有の、実装するうえで必要となる詳細な設計内容やコードレベルのインターフェース定義を機能ごとに分割して格納しています。

### 記載項目（標準テンプレート）
各詳細設計書には、以下の項目を標準として記載し、アーキテクチャや処理フローを視覚的かつ明確に定義します。

1. **対象機能の概要・処理一覧**
   - 機能の目的と、どのような処理で構成されているかをリストアップします。
2. **対象機能のモジュール構成図・クラス図**
   - **モジュール構成図 (Mermaid)**: レイヤー間（Router -> UseCase -> Interface <- Gateway 等）の依存関係を示します。
   - **クラス図 (Mermaid)**: 主要なドメインモデル（Entity/Pydanticスキーマ）や抽象インターフェースの構造を示します。
3. **対象機能の処理フロー図・シーケンス図**
   - **処理フロー図 / シーケンス図 (Mermaid)**: 正常系の主要な処理手順や、コンポーネント間の呼び出し順序を示します。
4. **APIインターフェース仕様 / 入出力データ（スキーマ）**
   - エンドポイントの定義、リクエスト/レスポンスの構造、バリデーションの制約等を記載します。
5. **異常系・エラーハンドリング**
   - 想定されるエラー（外部APIのタイムアウト、バリデーションエラー等）と、システムとしての対応方針（リトライ、フォールバック等）を記載します。
6. **依存する環境変数・外部設定**
   - 当該機能が動作するために必要な環境変数や、外部サービスの要件等を記載します。
7. **テスト方針**
   - 機能の検証アプローチ（単体テスト、結合テストの観点や、モック化すべき外部依存）を定義します。
8. **データモデル設計 (Data Model / Storage)**
   - 当該機能が所有し管理するデータベーステーブルのER図、およびテーブルスキーマを記載します。

### 対象ファイル一覧

- **`01_workflow_orchestrator.md`**
  - ワークフロー制御エンジンによるパイプラインのステート管理、非同期タスクキューの連動、エラーリカバリ、およびドキュメント収集の起点処理の詳細。
- **`02_document_render.md`**
  - ドキュメントを画像化する処理（Document Render API）の実装詳細。
- **`03_vision_extraction.md`**
  - 画像からの構造化テキスト抽出（Vision Extraction API）のインターフェースとGateway実装。
- **`04_ontology_generation.md`**
  - LLMを用いたオントロジー生成（Ontology Generation API）のインターフェース、Pydanticスキーマ定義、およびGateway実装。
- **`05_ontology_linking.md`**
  - 新規抽出されたオントロジーと既存グラフとの関連付け・リンク生成を行うエンジンの設計。
- **`06_ontology_evolution.md`**
  - 未分類概念の判定と提案を行うAI Agent（Ontology Evolution Agent）の設計。
- **`07_schema_compiler.md`**
  - スキーマ進化提案をPydantic/OWLファイルに反映するSchema Compilerの実装設計。
- **`08_schema_evolution_api.md`**
  - スキーマ進化提案に対するユーザーの承認・却下等のアクションを受け付けるAPIおよび専用UI画面の設計。
- **`09_graph_repository.md`**
  - データベース非依存を担保するためのGraph Repositoryインターフェース（DI）設計。
- **`10_console_ui.md`**
  - ユーザーが各サブ機能へアクセスするためのポータル画面（シェル）としての役割を担うSPA基盤の設計。
- **`11_graph_visualization.md`**
  - 蓄積されたノードとエッジを動的に探索・可視化するネットワークグラフUIの設計。
- **`12_document_management.md`**
  - ワークフローの起点となるドキュメントの登録、一覧表示、およびステータス監視を行うUIとAPI機能の設計。
